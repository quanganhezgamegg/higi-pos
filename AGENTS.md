...
- **Trạng thái hiện tại:** M0 (nền tảng & CI/CD), M1 (quản lý menu), và M2 (bàn & sơ đồ) đã hoàn tất, merge vào `main` local. M1 đã release production tag `v0.2.0`; M2 đã bump version `0.3.0` và build thử local. Các milestone M3–M7 chưa code; kế hoạch M2–M7 nằm trong `docs/superpowers/plans/`.
